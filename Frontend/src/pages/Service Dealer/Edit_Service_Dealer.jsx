...
const Edit_Service_Dealer = () => {
  ...
  const Getview = async (Eid) => {
    const result = await API.post(`/srevice_center_get_dealer_ID/${params.id}` , {} , {headers: { Authorization: `Bearer ${token}` }});
    setData({
    ...
    Brand_dropdown(result.data.Data[0].type)
...
const Payment_hendler = async (e) => {
  ...
  const Submite = async () => {
    console.log('Data.city_id', Data.name)
    if(
      Data.name== "" ||
      Data.address== "" ||
      Data.zipcode== "" ||
      ...
      const Result = await API.post(`/srevice_update_dealer/${params.id}`, Form, { headers: { Authorization: `Bearer ${token}` } })
      if (Result) {
