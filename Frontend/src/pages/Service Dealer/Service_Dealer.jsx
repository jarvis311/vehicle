...
const Service_Dealer = () => {
    ...
    swalWithBootstrapButtons
      ...
      .then(async (result) => {
        if (result.isConfirmed) {
          const form = new FormData()
          form.append('id', id)
          await API.post(`/srevice_delete_dealer`, form, { headers: { Authorization: `Bearer ${token}` } });
          GetData();
        } else {
          count = 10
          clearInterval(swalCountdownInterval)
  ...
  const Seaching = async (e, name) => {
    ...
    const result = await API.post("/searching_dealer", Form, { headers: { Authorization: `Bearer ${token}` } });
    setData(result.data.Data);
  ...
  const Togglechange = async (id, e, name) => {
    ...
    const result = await API.post("/toggle_service_dealer", Form, { headers: { Authorization: `Bearer ${token}` } });
    if (result) {
    ...
    <Layout sidebar={true}>
      <div className="page-heading">
        ...
        /> */}
          ...
          <Link to="/Add/service_Dealer" className="my-1 ms-3">
                        ...
                        return (
                          <tr>
                            ...
                            <td className='text-center'>
                              <Link to={`/view/service_Dealer/${val._id}`}>
