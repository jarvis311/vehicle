...
const View_Driving_School_city = () => {
  ...
  const Dropdown_Name = async()=>{
      ...
      Result.data.Data.map((val,i)=>{
          DropDownArr.push({label:val?.state_id?.state_name , value:val._id})
  ...
  const Getview = async (Eid) => {
      const result = await API.post(`/get_city_ID/${Eid !== "" ? Eid : id._id}` , {} , {headers: { Authorization: `Bearer ${token}` }});
      console.log('result', result)
      SetData({
        _id:result.data.Data[0]._id,
        state_id:result.data.Data[0].state_id.state_name,
          ...
          if (result.isConfirmed) {
           ...
           const result =  await API.post("/delete_city", formdata , {headers: { Authorization: `Bearer ${token}` }});
            if(reloadId.length === 0){
              // window.location.replace(`http://localhost:3000/driving_city`)
              window.location.replace(`${process.env.REACT_APP_BASE_URL}driving_city`)
            }else{
              // window.location.replace(`http://localhost:3000/view/driving_city/${reloadId[0]}`)
  ...
  return (
    ...
    <div className="page-heading">
        ...
        <div className="page-heading-right">
            ...
            <Link to={`/Edit/driving_city/${id._id}`}>
