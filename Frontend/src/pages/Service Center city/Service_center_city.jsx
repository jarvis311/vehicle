...
const Service_center_city = () => {
  ...
  const State_name_dropdown = async () => {
    ...
    Result.data.Data.map((val) => {
      console.log('val', val)
      StateNAmeArray.push({ label: val.name, value: val._id })
    })
    ...
    swalWithBootstrapButtons
      ...
      .then(async (result) => {
        if (result.isConfirmed) {
          ...
          form.append('id', id)
          await API.post(`/srevice_delete_city`, form, { headers: { Authorization: `Bearer ${token}` } });
          GetData();
        } else {
          count = 10
          clearInterval(swalCountdownInterval)
  ...
  const Seaching = async (e, name) => {
    ...
    const result = await API.post("/searchin_city", Form, { headers: { Authorization: `Bearer ${token}` } });
    setData(result.data.Data);
  ...
  const Togglechange = async (id, e, name) => {
    ...
    const result = await API.post("/toggle_city", Form, { headers: { Authorization: `Bearer ${token}` } });
    if (result) {
                        ...
                        return (
                          <tr>
                            ...
                            <td className='text-center'>
                              <Link to={`/view/service_city/${val._id}`}>
