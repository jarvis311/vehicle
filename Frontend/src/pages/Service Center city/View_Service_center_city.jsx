...
const View_Service_center_city = () => {
  ...
  const Getview = async (Eid) => {
    const result = await API.post(`/srevice_get_city_ID/${Eid !== "" ? Eid : id._id}`, {}, { headers: { Authorization: `Bearer ${token}` } });
    console.log('result', result)
    SetData({
      _id: result.data.Data[0]._id,
      state_id: result.data.Data[0].state_id.name,
      name: result.data.Data[0].name,
      S_id: result.data.Data[0].state_id._id,
    });
  };
    ...
    swalWithBootstrapButtons
      ...
      .then(async (result) => {
        if (result.isConfirmed) {
          ...
          const result = await API.post("/srevice_delete_city", formdata, { headers: { Authorization: `Bearer ${token}` } });
  ...
  return (
    ...
    <div className="page-heading">
      ...
      <div className="page-heading-right">
        ...
        <Link to={`/Edit/service_city/${id._id}`}>
