...
const Vehicale_Brand = () => {
    ...
    const Togglechange = async (id, e, name) => {
        ...
        const result = await API.post("/toggle/brand", Form, { headers: { Authorization: `Bearer ${token}` } });
        if (result) {
        ...
        swalWithBootstrapButtons
            ...
            .then(async (result) => {
                if (result.isConfirmed) {
                    const form = new FormData()
                    form.append('id', id)
                    await API.post(`/delete/brand`, form, { headers: { Authorization: `Bearer ${token}` } });
                    GetData();
                } else {
                    count = 10
                    clearInterval(swalCountdownInterval)
    ...
    const Seaching = async (e, name) => {
        ...
        const result = await API.post("/searching/brand", Form, { headers: { Authorization: `Bearer ${token}` } });
        setData(result.data.data);
    ...
    return (
        <Layout sidebar={true}>
            <div className="page-heading">
                <h3 className="my-1">Vehicale Brand</h3>
                <div className="page-heading-right">
                    ...
                    <Link to="/Add/Vehicale_Brand" className="my-1 ms-3">
                                                ...
                                                return (
                                                    <tr>
                                                        ...
                                                        <td>
                                                            <Switch
                                                                onChange={(e) => {
                                                                    Togglechange(val._id, e, "is_popular");
                                                                }}
                                                                checked={val.is_popular === 1 ? true : false}
                                                                offColor="#C8C8C8"
                                                                onColor="#0093ed"
                                                                height={30}
                                                                width={70}
                                                        ...
                                                        <td className='text-center'>
                                                            <Link to={`/view/Vehicale_Brand/${val._id}`}>
