...
const View_Driving_school_state = () => {
  ...
  const Dropdown_Name = async()=>{
      const Result = await API.post("/get_state" ,{},{headers: { Authorization: `Bearer ${token}` }})
      DropDownArr = []
      reloadId = []
      Result.data.Data.map((val,i)=>{
          DropDownArr.push({label:val.state_name , value:val._id})
  ...
  const Getview = async (Eid) => {
      const result = await API.post(`/get_state_ID/${Eid !== "" ? Eid : id._id}` , {} , {headers: { Authorization: `Bearer ${token}` }});
      ...
      swalWithBootstrapButtons
        ...
        .then(async (result) => {
          if (result.isConfirmed) {
           ...
           const result =  await API.post("/delete_state", formdata , {headers: { Authorization: `Bearer ${token}` }});
            if(reloadId.length === 0){
              ...
              // window.location.replace(`http://localhost:3000/view/driving/${reloadId[0]}`)
