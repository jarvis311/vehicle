...
const View_service_center = () => {
  ...
  const Getview = async (Eid) => {
      const result = await API.post(`/srevice_center_get_data_ID/${Eid !== "" ? Eid : id._id}` , {} , {headers: { Authorization: `Bearer ${token}` }});
      console.log('result', result)
      SetData({
      ...
      swalWithBootstrapButtons
        ...
        .then(async (result) => {
          if (result.isConfirmed) {
           ...
           const result =  await API.post("/srevice_delete_data", formdata , {headers: { Authorization: `Bearer ${token}` }});
            if(reloadId.length === 0){
              ...
              // window.location.replace(`http://localhost:3000/view/service_center/${reloadId[0]}`)
              window.location.replace(`${process.env.REACT_APP_BASE_URL}view/service_center/${reloadId[0]}`)
            }
          }else{
            count = 10
            clearInterval(swalCountdownInterval)
  ...
  return (
    ...
    <div className="page-heading">
        ...
        <div className="page-heading-right">
            ...
            <Link to={`/Edit/service_center/${id._id}`}>
        ...
        <Card>
            <Card.Body>
                <Row>
                    ...
                    <Col md={4}>
                        <div className='mb-4'>
                            <p className='mb-0 fw-bold'>Contect</p>
