...
const View_Vehicale_body_type = () => {
  ...
  const Getview = async (Eid) => {
    const result = await API.post(`/vehicle/bodytype/${Eid !== "" ? Eid : id._id}`, {}, { headers: { Authorization: `Bearer ${token}` } });
    ...
    swalWithBootstrapButtons
      ...
      .then(async (result) => {
        if (result.isConfirmed) {
          ...
          const result = await API.post("/delete/vehicle/bodytype", formdata, { headers: { Authorization: `Bearer ${token}` } });
          if (reloadId.length === 0) {
            window.location.replace(`http://localhost:3000/Vehicale_body_type`)
            // window.location.replace(`${process.env.REACT_APP_BASE_URL}Vehicale_body_type`)
          } else {
            window.location.replace(`http://localhost:3000/view/Vehicale_body_type/${reloadId[0]}`)
            // window.location.replace(`${process.env.REACT_APP_BASE_URL}view/Vehicale_body_type/${reloadId[0]}`)
          }
        } else {
          count = 10
          clearInterval(swalCountdownInterval)
  ...
  return (
    ...
    <div className="page-heading">
      <h3><Link to="/Vehicale_body_type" className='btn btn-primary btn-icon-lg me-3'><i className='bx bxs-left-arrow-alt'></i></Link>View Body type</h3>
      <div className="page-heading-right">
        ...
        <Link to={`/Edit/Vehicale_body_type/${id._id}`}>
      ...
      <Card>
        <Card.Body>
          <Row>
            ...
            <Col md={4}>
              <div className='mb-4'>
                <p className='mb-0 fw-bold'>status</p><span>{Data.status == 1 ? "Open" : "Close"}</span>
              </div>
            </Col>
          </Row>
        </Card.Body>
...
export default View_Vehicale_body_type